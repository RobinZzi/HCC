...
dmr_sig_test_result$case = factor(dmr_sig_test_result$case, levels=c("Enriched in Hypo","Enriched in Hyper"))
  ggplot(dmr_sig_test_result,aes(x=lnR,y=logfdr))+
    ...
    theme_bw() +
    geom_text_repel(aes(label=histone_mark, color = case), size =5,hjust=0.6,vjust=-1)+
    theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank())+
    theme(axis.line = element_line(colour = "black"))+
    labs(title="Chi-square test result of Histone Mark in GADD45A-KD-HepG2(guangzhou)")+
