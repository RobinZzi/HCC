...
mtx <- as.data.frame(cbind(c(0,24.5,327.52,419.58,231.7),c(0.042,0.79,13.136,17.013,9.253)))

colnames(mtx) <- c("t","E")


f<-function(x){
  y<-0.0000003826723*(x^2)+0.039069*x-0.053373
...
ggplot(mtx,aes(t,E))+
  geom_point( colour = "#EDD04E")+geom_smooth(method = "lm",fill=NA,colour = "#D9250D",linetype = "dotted")+theme_bw()+
  xlab("温度t[°C]") + 
  ...
  geom_abline(slope=0.0405796, intercept=-0.0981023, colour = "#6894F2", size = 0.5,linetype = "solid")+
  geom_line(aes(x,y),color="#49b462",linetype = "longdash")


ggplot(mtx,aes(t,E))+
  geom_point( colour = "white",size=)+geom_smooth(method = "lm",fill=NA,colour = "white",linetype = "dotted")+theme_bw()+
  xlab("温度t[°C]") + 
  theme(axis.title.x = element_text(size = 15,
                                    face = "bold", 
                                    vjust = 0.5, 
  ...
  theme(panel.grid.major=element_line(colour=NA),
        ...
        panel.grid.minor = element_blank()) +
  theme(legend.position='none')  +
  geom_abline(slope=0.0405796, intercept=-0.0981023, colour = "white", size = 0.5,linetype = "solid")+
  geom_line(aes(x,y),color="#49b462",linetype = "longdash")
